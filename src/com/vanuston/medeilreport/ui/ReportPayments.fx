...
var log:Logger=Logger.getLogger(ReportPayments.class,"Reports");
    ...
    public-read def PanelPayments: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("FR6".trim())){
    ...
    function hyperlinkAction(): Void {
        ...
        params.put("type1",'todaypayment');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/payment.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Payments','{hyperlink.text}')");
        }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Payments List",msg);
        showAlertbox();
        }
    ...
    function button1Action(): Void {
        ...
        params.put("type1",'paymentbydate');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/payment.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Payments','{button1.text}')");
        }
        catch (e:Exception)
    {
        var msg:String="Class : ReportPayments method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
