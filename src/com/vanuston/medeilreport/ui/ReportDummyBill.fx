...
var log:Logger=Logger.getLogger(ReportDummyBill.class,"Reports");
var dir1 = new File(".");
...
var fxCalendar = FXCalendar {
    ...
    function rectangle2OnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
        ...
        var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("SR6".trim())){
        ...
        CustomAlert.ShowInfo("Supplementary Bill Report",msg);
        showAlertbox();
        }
    ...
    function imgFromOnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
        ...
        RepViewer.showReport("/reportfiles/sales_dummybill.jasper",params1);
       commonController.queryExecution("CALL pro_userlog('Supplementary Bill Report','{button1.text}')");
        }              
        }
    catch (e:Exception)
    {   var msg:String="Class : ReportDummyBill method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
