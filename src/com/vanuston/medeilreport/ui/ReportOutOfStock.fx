...
var log:Logger=Logger.getLogger(ReportOutOfStock.class,"Reports");
var dir1 = new File(".");
...
var commonController = RegistryFactory.getClientStub("CommonImplementStub") as CommonImplements;
    ...
    public-read def label2: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        text: "This report is used to display the products which has reached the level of zero.This report is an indication for the user to buy these products."
        font: Arial_16
        textWrap: true
    }
    ...
    // </editor-fold>//GEN-END:main
        ...
        var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("StkAR4".trim())){
        ...
        params.put("type1",'out_of_stock');
        params.put("title",'OUT OF STOCK LIST');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/min_stock.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Out of Stock','{button1.text}')");
        }
        catch (e:Exception)
        {
        var msg:String="Class : ReportOutOfStock method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
