...
public class ReportCreditnotes
{
...
var log:Logger=Logger.getLogger(ReportCreditnotes.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    public-read def hyperlink: javafx.scene.control.Hyperlink = javafx.scene.control.Hyperlink {
        ...
        graphic: imageView2
        text: "Today Credit Notes"
        font: Arial_16
        graphicHPos: javafx.geometry.HPos.RIGHT
        action: hyperlinkAction
    }
    
    public-read def PanelCreditnotes: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, label5, label10, txtFromDate, txtToDate, button1, FromPanel, imgFrom, imgTo, hyperlink, panelAlert, panel, ]
    }
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("FR2".trim())){
    ...
    function hyperlinkAction(): Void {
        ...
        params1.put("type1",'todaycreditnote');
        params1.put("REPORTS_DIR",image);   
        RepViewer.showReport("/reportfiles/credit_note.jasper",params1);
       commonController.queryExecution("CALL pro_userlog('Credit Notes','{button1.text}')");
         }
    ...
    CustomAlert.ShowInfo("Credit Notes List",msg);
    showAlertbox();
    }
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
       ...
       if(not toFlag)
        ...
        params1.put("type1",'creditnotebydate');
        params1.put("REPORTS_DIR",image);        
        RepViewer.showReport("/reportfiles/credit_note.jasper",params1);        
       commonController.queryExecution("CALL pro_userlog('Credit Notes','{button1.text}')");
        }
    ...
    {
        var msg:String="Class : ReportCreditnotes method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
