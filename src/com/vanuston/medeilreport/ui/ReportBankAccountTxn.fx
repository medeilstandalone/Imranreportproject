...
public class ReportBankAccountTxn
{
...
var log:Logger=Logger.getLogger(ReportBankAccountTxn.class,"Reports");
var dir1 = new File(".");
...
var accountNo:String[]=commonController.AccountNumber();
    ...
    public-read def label: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        effect: null
        text: "Accountwise Transaction Details"
        font: Arial_25
    ...
    public-read def PanelBankTxn: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, label5, label2, label10, txtFromDate, txtToDate, button1, imgFrom, imgTo, FromPanel, hyperlink, cboAccno, panelAlert, panel, ]
    }
    ...
    function hyperlinkAction(): Void {
        if(cboAccno.selectedItem==accountNo[0])
        {
        ...
        params.put("accnumber",cboAccno.selectedItem);
        params.put("fromdate",datetime);
        params.put("todate",datetime);
        RepViewer.showReport("/reportfiles/bankacc_txn.jasper",params);
        commonController.queryExecution("CALL pro_userlog('Bank Account Txns','{hyperlink.text}')");
        }
      }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Bank Account Transactions",msg);
        showAlertbox();
        }
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
       ...
       frmFlag=false;
        ...
        else if(cboAccno.selectedItem==accountNo[0])
        {
        FXinfo("Please select the Account Number");
        }
        else
        ...
        RepViewer.showReport("/reportfiles/bankacc_txn.jasper",params);
        commonController.queryExecution("CALL pro_userlog('Bank Account Txns','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportBankAccountTxn method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
