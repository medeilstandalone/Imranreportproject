...
public class ReportDamagedStock
{
...
var fxCalendar = FXCalendar {
    ...
    FromPanel.visible=false;
        ...
        CustomAlert.ShowInfo("Damaged Stock List",msg);
        showAlertbox();
        }
    ...
    function imgFromOnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
        ...
        RepViewer.showReport("/reportfiles/damage_stock.jasper",params1);
       commonController.queryExecution("CALL pro_userlog('Damaged Stock','{button1.text}')");
        }              
        }
    catch (e:Exception)
    {   var msg:String="Class : ReportDamagedStock method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
