...
var log:Logger=Logger.getLogger(ReportDebitnotes.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    public-read def PanelDebitNote: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("FR3".trim())){
    ...
    function hyperlinkAction (): Void {
        ...
        params1.put("type1",'todaydebitnote');
        params1.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/debit_note.jasper",params1);
       commonController.queryExecution("CALL pro_userlog('Debit Notes','{hyperlink.text}')");
    }
   ...
   function imgFromOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
       ...
       if(not toFlag)
        ...
        params1.put("REPORTS_DIR",image);
        params1.put("type1",'debitnotebydate');
        RepViewer.showReport("/reportfiles/debit_note.jasper",params1);
       commonController.queryExecution("CALL pro_userlog('Debit Notes','{button1.text}')");
        }
     catch (e:Exception)
    {   var msg:String="Class : ReportDebitnotes method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
