...
public class ReportEmployeeSalary
{
...
var log:Logger=Logger.getLogger(ReportEmployeeSalary.class,"Reports");
var dir1 = new File(".");
...
var fxCalendar = FXCalendar {
    ...
    public-read def PanelEmployeeSalary: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("MR9".trim())){
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Employee Salary List",msg);
        showAlertbox();
        }
    ...
    function imgFromOnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
        ...
        RepViewer.showReport("/reportfiles/employee_salary.jasper",params1);
       commonController.queryExecution("CALL pro_userlog('Employee Salary','{button1.text}')");
        }                    
        }
    catch (e:Exception)
    {
        var msg:String="Class : ReportEmployeeSalary method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
    }
    }
