...
public class ReportDistributorPayments
{
...
var log:Logger=Logger.getLogger(ReportDistributorPayments.class,"Reports");
var dir1 = new File(".");
...
var fxCalendar = FXCalendar {
    ...
    public-read def label: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        effect: null
        text: "Payment History by Distributor"
        font: Arial_25
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("MR4".trim())){
    ...
    shortcut(event.code);
        ...
        CustomAlert.ShowInfo("Distributor Payments",msg);
        showAlertbox();
        }
    ...
    function button1Action(): Void {
        ...
        RepViewer.showReport("/reportfiles/dist_payment.jasper",params1);
       commonController.queryExecution("CALL pro_userlog('Distributor Payments','{button1.text}')");
        }      
        catch (e:Exception)
    {
        var msg:String="Class : ReportDistributorPayments method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
