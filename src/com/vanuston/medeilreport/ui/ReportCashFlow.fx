...
public class ReportCashFlow {
    ...
    public-read def imgTo: javafx.scene.image.ImageView = javafx.scene.image.ImageView {
        ...
        layoutY: 285.0
        onMouseClicked: imageToOnMouseClicked
        blocksMouse: true
        image: image2
        fitWidth: 25.0
        fitHeight: 25.0
    ...
    public-read def PanelCashFlow: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, label5, label10, txtFromDate, txtToDate, button2, FromPanel, imgFrom, imgTo, panelAlert, panel, ]
    }
    ...
    function FXinfo(msg: String): Void {
        CustomAlert.ShowInfo("Cash Flow", msg);
        showAlertbox();
    }
    ...
    function imageToOnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
    ...
    function button2Action(): Void {
        ...
        commonController.querySelection("call pro_cashflow('{sd}','{ed}',@1)");
        ...
        RepViewer.showReport("/reportfiles/cash_flow.jasper",params1);     
        commonController.queryExecution("CALL pro_userlog('Cash Flow','{button2.text}')");
        }
        
    catch (e:Exception)
    {
        var msg:String="Class : ReportCashFlow  method : button2Action()   = {e.getMessage()}";
        log.debug(msg);
