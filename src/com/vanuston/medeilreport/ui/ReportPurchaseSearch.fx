...
public class ReportPurchaseSearch
{
...
var log:Logger=Logger.getLogger(ReportPurchaseSearch.class,"Reports");
var dir1 = new File(".");
...
var fxCalendar = FXCalendar {
    ...
    public-read def rdoinvsearch: javafx.scene.control.RadioButton = javafx.scene.control.RadioButton {
        layoutX: bind C1LX
        layoutY: 155.0
    ...
    public-read def PanelCustomerLedger: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, ToDate, FromDate, txtFromDate, txtToDate, button1, txtInvoiceNumber, invoiceNumber, rdoinvsearch, rdobatchsearch, rdoproductsearch, imgFrom, imgTo, imageView, FromPanel, listInvoiceNumber, panelAlert, panel, ]
    }
    ...
    function rdoinvsearchOnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
        invoiceNumber.text = "Enter Invoice Number";
    ...
    CustomAlert.ShowInfo("Purchase Search",msg);
    showAlertbox();
    }
   ...
   function imgToOnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
    ...
    function button1Action(): Void {
      ...
      if(rdoinvsearch.selected == true) {
        ...
        params1.put("invoiceno",number);
        RepViewer.showReport("/reportfiles/purchaseinvoicesearch.jasper",params1);
        }
      }
        ...
        params1.put("fromdate",'0000-00-00');
        params1.put("todate",'0000-00-00');
        RepViewer.showReport("/reportfiles/purchaseproductsearch.jasper",params1);
        }
      }
       ...
       commonController.queryExecution("CALL pro_userlog('Purchase Invoice Search','{button1.text}')");
        }
        catch (e:Exception)
        {
     ...
     function getInvoice(kcode: javafx.scene.input.KeyCode,pname:String) {
        ...
        if(rdoinvsearch.selected == true) {
        invoiceNumber =  commonController.invoiceNumber(pname).toArray();
        }
        else if(rdobatchsearch.selected == true) {
