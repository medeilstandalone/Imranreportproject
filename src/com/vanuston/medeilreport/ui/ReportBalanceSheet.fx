...
var log:Logger=Logger.getLogger(ReportBalanceSheet.class,"Reports");
var sd:String;
var ed:String;
...
var commonController = RegistryFactory.getClientStub("CommonImplementStub") as CommonImplements;
    ...
    public-read def PanelBalanceSheet: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        blocksMouse: false
    ...
    CustomAlert.ShowInfo("Balance Sheet",msg);
    showAlertbox();
    }
    ...
    insert RepViewer.getDesignRootNodes() into panel.content;
        ...
        commonController.querySelection("call pro_balancesheet('{sd}','{ed}')");
        RepViewer.showReport("/reportfiles/balance_sheet.jasper",params1);        
        commonController.queryExecution("CALL pro_userlog('Balance Sheet','{button2.text}')");
        }

    catch (e:Exception)
    {
    var msg:String="Class : ReportBalanceSheet  method : button2Action()   = {e.getMessage()}";
    log.debug(msg);
