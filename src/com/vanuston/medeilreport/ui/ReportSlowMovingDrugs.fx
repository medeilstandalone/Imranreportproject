...
public class ReportSlowMovingDrugs
{
...
var fxCalendar = FXCalendar {
 ...
 var log:Logger=Logger.getLogger(ReportSlowMovingDrugs.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    public-read def label: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        effect: null
        text: "Slow Moving Product Details"
        font: Arial_25
    ...
    public-read def PanelSlowMoving: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("StkAR2".trim())){
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Slow Moving Drugs",msg);
        showAlertbox();
        }
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
        ...
        params.put("type1",'slow_moving');
        params.put("fromdate",sd);
        params.put("todate",ed);
        params.put("title",'SLOW MOVING PRODUCT DETAILS');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/stock_analysis.jasper",params);
        commonController.queryExecution("CALL pro_userlog('Slow Moving Drugs','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportSlowMovingDrugs method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
