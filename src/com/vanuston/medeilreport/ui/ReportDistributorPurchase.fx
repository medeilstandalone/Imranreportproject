...
public class ReportDistributorPurchase
{
...
var log:Logger=Logger.getLogger(ReportDistributorPurchase.class,"Reports");
var sd:String;
var ed:String;
var s:String;
var dir1 = new File(".");
...
var commonController = RegistryFactory.getClientStub("CommonImplementStub") as CommonImplements;
    ...
    public-read def label: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        effect: null
        text: "Purchase Invoice by Distributor Details"
        font: Arial_25
    ...
    public-read def PanelDistributorLedger: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, ToDate, FromDate, txtFromDate, txtToDate, button1, DistName, DistributorName, imgFrom, imgTo, FromPanel, listDistributorName, imageView, hyperlink, panelAlert, panel, ]
    }
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("MR3".trim())){
    ...
    function hyperlinkAction(): Void {
        ...
        params.put("distname",DistName.text);
        params.put("type1",'distpurchase');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/purchase_detailreport.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Distributor Purchase Report','{hyperlink.text}')");
        }
        }
    ...
    function txtFromDateOnKeyPressed (event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Distributor Purchase",msg);
        showAlertbox();
        }
    ...
    function button1Action(): Void {
        ...
        RepViewer.showReport("/reportfiles/dist_purchase.jasper",params1);
       commonController.queryExecution("CALL pro_userlog('Distributor Purchase','{button1.text}')");
        }
        
    catch (e:Exception)
    {
        var msg:String="Class : ReportDistributorPurchase method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
