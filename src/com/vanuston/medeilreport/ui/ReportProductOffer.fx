...
public class ReportProductOffer
{
...
var log:Logger=Logger.getLogger(ReportProductOffer.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    public-read def PanelProductOffer: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("PR2".trim())){
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Product Offer List",msg);
        showAlertbox();
        }
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
        ...
        RepViewer.showReport("/reportfiles/item_offer.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Product Offer','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportProductOffer method : button1Action()  = {e.getMessage()}";
        log.debug(msg);
