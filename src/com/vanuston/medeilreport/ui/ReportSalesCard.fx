...
var log:Logger=Logger.getLogger(ReportSalesCard.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    public-read def PanelCardTxn: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
    ...
    function hyperlinkAction(): Void {
        ...
        RepViewer.showReport("/reportfiles/card_txn.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Card Transaction','{hyperlink.text}')");
        }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Card Transactions",msg);
        showAlertbox();
        }
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
       ...
       frmFlag=false;
        ...
        RepViewer.showReport("/reportfiles/card_txn.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Card Transactions','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportCardTxn method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
