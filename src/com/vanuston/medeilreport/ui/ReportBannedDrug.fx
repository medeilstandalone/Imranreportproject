...
var log:Logger=Logger.getLogger(ReportBannedDrug.class,"Reports");
    ...
    public-read def label2: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        text: "This report is used to display the list of banned drugs. The list includes the product\'s generic Name, Therapeutic area,Formulation and the date when it was banned. Latest Banned drugs will also get included in this list on a regular basis."
        font: Arial_16
        textWrap: true
    }
   ...
   var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("PR3".trim())){
...
def RepViewer = ReportViewer{
    heading: bind label.text;
        ...
        function button1Action(): Void {
            ...
            RepViewer.showReport("/reportfiles/banned_drug.jasper",params1);
            commonController.queryExecution("CALL pro_userlog('Banned Drug','{button1.text}')");
            }
            catch (e:Exception)
            {
            var msg:String="Class : ReportBannedDrug method : button1Action()   = {e.getMessage()}";
            log.debug(msg);            
            }
            }
