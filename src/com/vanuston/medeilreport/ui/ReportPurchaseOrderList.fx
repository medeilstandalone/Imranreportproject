...
public class ReportPurchaseOrderList
{
...
var log:Logger=Logger.getLogger(ReportPurchaseOrderList.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("PurR3".trim())){
    ...
    function hyperlinkAction(): Void {
        ...
        params.put("type1",'todaypo');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/purchase_order.jasper",params);
        commonController.queryExecution("CALL pro_userlog('Purchase Order','{hyperlink.text}')");
         }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Purchase Order List",msg);
        showAlertbox();
        }
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
        ...
        params.put("type1",'pobydate');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/purchase_order.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Purchase Order','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportPurchaseOrderList method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
