...
var log:Logger=Logger.getLogger(ReportCustomerList.class,"Reports");
var dir1 = new File(".");
...
var commonController = RegistryFactory.getClientStub("CommonImplementStub") as CommonImplements;
    ...
    public-read def label2: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        text: "This report displays the list of the customers who have visited the shop along with their details such as Address,Phone number, Email ID and alert type."
        font: Arial_16
        textWrap: true
    }
    ...
    public-read def PanelCustomerList: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
    ...
    // </editor-fold>//GEN-END:main
        var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("CR1".trim())){
        ...
        try
        {
            ...
            RepViewer.showReport("/reportfiles/customer_info.jasper",params1);
            commonController.queryExecution("CALL pro_userlog('Customer List','{button1.text}')");
        }
        catch (e:Exception)
        {
            var msg:String="Class : ReportCustomerList method : button1Action() = {e.getMessage()}";
            log.debug(msg);
        }
        }
        }
