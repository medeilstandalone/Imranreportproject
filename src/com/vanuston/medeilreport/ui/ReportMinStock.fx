...
public class ReportMinStock
{
var log:Logger=Logger.getLogger(ReportMinStock.class,"Reports");
var dir1 = new File(".");
...
var LX=bind (panelW-textW)/2;
    ...
    public-read def label2: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        text: "This report lists the products which has reached  minimum level of stock in the store.It consists of item name and quantity of those products."
        font: Arial_16
        textWrap: true
    }
    ...
    // </editor-fold>//GEN-END:main
        ...
        var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("StkAR3".trim())){
        ...
        params.put("type1",'min_stock');
        params.put("title",'MINIMUM STOCK DETAILS ');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/min_stock.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Minimum Stock','{button1.text}')");
        }
        catch (e:Exception)
        {
        var msg:String="Class : ReportMinStock method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
