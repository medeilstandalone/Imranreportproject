...
public class ReportPurchasereturn
{
...
var log:Logger=Logger.getLogger(ReportPurchasereturn.class,"Reports");
var dir1 = new File(".");
...
var listH=0.0;
    ...
    public-read def rdoDistributor: javafx.scene.control.RadioButton = javafx.scene.control.RadioButton {
        layoutX: bind C1LX-20
        layoutY: 145.0
        onMouseClicked: rdoDistributorOnMouseClicked
        text: "Search by Distributor"
        font: Arial_14
        toggleGroup: toggleGroup
        selected: true
    }
    ...
    def __layoutInfo_imageView3: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
        height: 0.0
    }
    public-read def imageView3: javafx.scene.image.ImageView = javafx.scene.image.ImageView {
        layoutX: bind C1LX+100+Type2TextboxW-2
        layoutY: 210.0
        layoutInfo: __layoutInfo_imageView3
        onMouseClicked: imageView3OnMouseClicked
        image: image
        fitHeight: 25.0
    }
    
    public-read def PanelPurchasereturn: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, label5, label10, txtFromDate, txtToDate, button1, imgFrom, imgTo, DistName, label2, FromPanel, imageView3, listDistributorName, rdoDistributor, rdoProduct, hyperlink, panel, panelAlert, ]
    }
    ...
    function rdoDistributorOnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
        label2.text = "Distributor Name";
        DistName.text = "";
    }

    function imageView3OnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("PurR4".trim())){
    ...
    function hyperlinkAction(): Void {
        ...
        params.put("type1",'todaypurchaseret');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/purchase_return.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Purchase Return','{hyperlink.text}')");
        }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Purchase Return List",msg);
        showAlertbox();
        }
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
        ...
        if(DistName.text.trim().equals("")) {
        params.put("type1",'purchaseretbydate');
        params.put("distname","");
        }
        else {
            if(rdoDistributor.selected==true) {
            params.put("type1",'distpurchaseret');
            }
            else {
            params.put("type1",'productpurchaseret');
            }
            params.put("distname",DistName.text.trim());
        }
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/purchase_return.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Purchase Return','{button1.text}')");
        }
        catch (e:Exception)
        {
        var msg:String="Class : ReportPurchasereturn method : button1Action()   = {e.getMessage()}";
        log.debug(msg);        
        }
    }
   }
  
function getDistributor(kcode: javafx.scene.input.KeyCode,cname:String) {
        var distName: String[] = [];
       listDistributorName.visible = true;
       if(rdoDistributor.selected == true) {
       distName =  commonController.DistributorName(cname);
       }
       else {
       distName =   commonController.ItemName(cname);
       }
