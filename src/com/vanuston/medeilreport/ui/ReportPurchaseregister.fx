...
public class ReportPurchaseregister
{
...
var log:Logger=Logger.getLogger(ReportPurchaseregister.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    public-read def PanelPurchaseRegister: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, label5, label10, txtFromDate, txtToDate, button1, imgFrom, imgTo, hyperlink, hyperlink2, FromPanel, panelAlert, panel, ]
    }
    ...
    function hyperlink2Action(): Void {
        ...
        RepViewer.showReport("/reportfiles/purchase_invoicereport.jasper",params);
        commonController.queryExecution("CALL pro_userlog('Purchase Register','{button1.text}')");
        }
         }
         catch(e:Exception) {
          var msg:String="Class : ReportPurchaseregister method : button1Action()  = {e.getMessage()}";
          log.debug(msg);
         }
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("PurR2".trim())){
    ...
    function hyperlinkAction (): Void {
        ...
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/purchaseregister.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Purchase Register','{button1.text}')");
    }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Purchase Register",msg);
        showAlertbox();
        }
