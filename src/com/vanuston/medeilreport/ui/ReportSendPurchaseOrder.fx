...
public class ReportSendPurchaseOrder
{
...
var log:Logger=Logger.getLogger(ReportSendPurchaseOrder.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    public-read def label: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        effect: null
        text: "Sent Purchase Orders List"
        font: Arial_25
    ...
    public-read def PanelPurchaseOrder: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, label5, label10, txtFromDate, txtToDate, button1, imgFrom, imgTo, FromPanel, panelAlert, panel, ]
    }
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("PurR5".trim())){
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Sent Purchase Orders List",msg);
        showAlertbox();
        }
       ...
       if(not frmFlag)
        {
            ...
            if(txtFromDate.rawText!=null)
            {
                var dat = txtFromDate.rawText.split("-");
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
        ...
        if(not toFlag)
        {
           ...
           panelCalenderLY = imgTo.layoutY-30.0;
            ...
            if(txtToDate.rawText!=null)
            {
                var dat = txtToDate.rawText.split("-");
                var d = Integer.parseInt(dat[0]);
                var m = Integer.parseInt(dat[1])-1;
                var y = Integer.parseInt(dat[2]);
                fxCalendar.set(y,m,d);
        ...
        if (txtFromDate.text==null or txtFromDate.text.length()==0)
        {
        FXinfo("Please Enter the From Date");
        }else    if (txtToDate.text==null or txtToDate.text.length()==0)
        {
        ...
        RepViewer.showReport("/reportfiles/sent_purchaseorder.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Sent Purchase Orders','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportSendPurchaseOrder method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
