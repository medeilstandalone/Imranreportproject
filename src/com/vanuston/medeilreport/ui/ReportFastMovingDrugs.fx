...
public class ReportFastMovingDrugs
{
...
var log:Logger=Logger.getLogger(ReportFastMovingDrugs.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    public-read def label: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        effect: null
        text: "Fast Moving Product Details"
        font: Arial_25
    ...
    public-read def PanelFastMoving: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, label5, label10, txtFromDate, txtToDate, button1, imgFrom, imgTo, FromPanel, hyperlink, panelAlert, panel, ]
    }
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("StkAR1".trim())){
    ...
    function hyperlinkAction(): Void {
        ...
        params.put("type1",'todayfast_moving');
        params.put("fromdate",'0000-00-00');
        params.put("todate",'0000-00-00');
        params.put("title",'FAST MOVING PRODUCT DETAILS');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/stock_analysis.jasper",params);
        commonController.queryExecution("CALL pro_userlog('Fast Moving Drugs','{hyperlink.text}')");
        }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Fast Moving Drugs",msg);
        showAlertbox();
        }
   ...
   function imgFromOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
        ...
        var image:String="{path}""/reportfiles/Medil Logo.png";
        var params  = new HashMap();
        params.put("type1",'fast_moving');
        params.put("fromdate",sd);
        params.put("todate",ed);
        params.put("title",'FAST MOVING PRODUCT DETAILS');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/stock_analysis.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Fast Moving Drugs','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportFastMovingDrugs method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
