...
var log:Logger=Logger.getLogger(ReportSalesReturn.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    public-read def PanelSalesReturn: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("SR3".trim())){
    ...
    function hyperlinkAction(): Void {
        ...
        params.put("type1",'todaysalesret');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/sales_return.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Sales Return','{hyperlink.text}')");
         }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Sales Return List",msg);
        showAlertbox();
        }
    ...
    function imgToOnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
       ...
       if(not toFlag)
        ...
        params.put("type1",'salesretbydate');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/sales_return.jasper",params);
        commonController.queryExecution("CALL pro_userlog('Sales Return','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportSalesReturn method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
