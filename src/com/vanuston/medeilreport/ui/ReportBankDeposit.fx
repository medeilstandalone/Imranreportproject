...
var log:Logger=Logger.getLogger(ReportBankDeposit.class,"Reports");
var dir1 = new File(".");
...
var accountNo:String[]=commonController.AccountNumber();
    ...
    function hyperlinkAction(): Void {
        ...
        RepViewer.showReport("/reportfiles/bankacc_deposit.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Bank Deposit details','{hyperlink.text}')");
        }
      }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Bank Deposit Details",msg);
        showAlertbox();
        }
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
       ...
       frmFlag=false;
        ...
        RepViewer.showReport("/reportfiles/bankacc_deposit.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Bank Deposit Details','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportBankDeposit method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
