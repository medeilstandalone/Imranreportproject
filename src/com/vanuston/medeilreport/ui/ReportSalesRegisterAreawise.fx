...
public class ReportSalesRegisterAreawise
{
...
var log:Logger=Logger.getLogger(ReportSalesRegisterAreawise.class,"Reports");
var dir1 = new File(".");
...
var fxCalendar = FXCalendar {
    ...
    def __layoutInfo_listCustAddress: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
        width: bind Type1TextboxW
        height: bind listH
    }
    public-read def listCustAddress: javafx.scene.control.ListView = javafx.scene.control.ListView {
        visible: false
        layoutX: bind C1LX+125
        layoutY: 173.0
        layoutInfo: __layoutInfo_listCustAddress
        onKeyPressed: listCustAddressOnKeyPressed
    ...
    public-read def ProdName: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        text: "Enter the Area/Location"
        font: Arial_14
    }
    
    def __layoutInfo_custAddress: javafx.scene.layout.LayoutInfo = javafx.scene.layout.LayoutInfo {
        width: bind Type1TextboxW
    }
    public-read def custAddress: javafx.scene.control.TextBox = javafx.scene.control.TextBox {
        visible: true
        layoutX: bind C1LX+125
        layoutY: 150.0
        layoutInfo: __layoutInfo_custAddress
        onKeyPressed: custAddressOnKeyPressed
        onKeyReleased: custAddressOnKeyReleased
        text: ""
        font: Arial_14
    }
    ...
    public-read def PanelCustomerLedger: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, ToDate, FromDate, txtFromDate, txtToDate, button1, custAddress, ProdName, imgFrom, imgTo, FromPanel, listCustAddress, imageView, panelAlert, panel, ]
    }
    ...
    CustomAlert.ShowInfo("Sales Register Areawise",msg);
    showAlertbox();
    }
    ...
    function imageViewOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
        getCustomerAddress(KeyCode.VK_DOWN,"");
        }
        

    function listCustAddressOnKeyPressed (event: javafx.scene.input.KeyEvent): Void {
                
        if(listCustAddress.focused)
            if (event.code == KeyCode.VK_DOWN)
        listCustAddress.selectFirstRow();
    }

    function custAddressOnKeyReleased (event: javafx.scene.input.KeyEvent): Void {
         if (event.code != KeyCode.VK_F10)
          {
          getCustomerAddress(event.code,custAddress.rawText.trim());
            if (event.code == KeyCode.VK_DOWN)
                listCustAddress.requestFocus();
                listCustAddress.selectFirstRow();
                }
    ...
    function custAddressOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
   ...
   function imgToOnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
    ...
    function button1Action(): Void {
        if(custAddress.text==null or custAddress.text.trim().length()==0)
        {
        FXinfo("Please Enter the Area/Location");
        }
        ...
        params1.put("location",custAddress.rawText.trim());
        RepViewer.showReport("/reportfiles/salesRegisterAreawise.jasper",params1);
       commonController.queryExecution("CALL pro_userlog('Sales Register Areawise','{button1.text}')");
        }        
        catch (e:Exception)
    {   var msg:String="Class : ReportSalesRegisterAreawise method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
        log.debug(e);
    }
}
     ...
     function getCustomerAddress(kcode: javafx.scene.input.KeyCode,custAdd:String) {
        var custAddress1: String[] = [];
        listCustAddress.visible = true;
        custAddress1 =  commonController.customerAddress(custAdd);
        listCustAddress.clearSelection();
        listCustAddress.items = custAddress1;
        var size =  listCustAddress.items.size();
        listCustAddress.onKeyPressed = function(e) {
                    if (e.code == KeyCode.VK_ENTER) {
                        custAddress.text = "{listCustAddress.selectedItem}";
                        listCustAddress.visible = false;
                        txtFromDate.requestFocus();
                    } else if (e.code == KeyCode.VK_DOWN) {
                        if (listCustAddress.selectedIndex == 0) {
                            listCustAddress.selectFirstRow();
                        }
        ...
        listCustAddress.onMouseClicked = function(e) {
                    custAddress.text = "{listCustAddress.selectedItem}";
                    listCustAddress.visible = false;
                    txtFromDate.requestFocus();
                }
