...
var log:Logger=Logger.getLogger(ReportReceipts.class,"Reports");
var dir1 = new File(".");
...
var fxCalendar = FXCalendar {
    ...
    public-read def PanelReceipts: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("FR7".trim())){
    ...
    function hyperlinkAction(): Void {
        ...
        params.put("type1",'todayreceipt');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/receipts.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Receipts','{hyperlink.text}')");
         }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Receipts List",msg);
        showAlertbox();
        }
    ...
    function button1Action(): Void {
        ...
        params.put("type1",'receiptbydate');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/receipts.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Receipts','{button1.text}')");
        }           
        catch (e:Exception)
    {
        var msg:String="Class : ReportReceipts method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
