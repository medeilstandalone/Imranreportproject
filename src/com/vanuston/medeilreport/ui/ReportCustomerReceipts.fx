...
public class ReportCustomerReceipts
{
...
var log:Logger=Logger.getLogger(ReportCustomerReceipts.class,"Reports");
var dir1 = new File(".");
...
var fxCalendar = FXCalendar {
    ...
    public-read def PanelCustomerReceipts: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, ToDate, FromDate, txtFromDate, txtToDate, button1, CustName, CustomerName, imgFrom, imgTo, FromPanel, listCustomerName, imageView, panelAlert, panel, ]
    }
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("SR5".trim())){
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Customer Receipts",msg);
        showAlertbox();
        }
    ...
    function button1Action(): Void {
        ...
        RepViewer.showReport("/reportfiles/cust_receipt.jasper",params1);
       commonController.queryExecution("CALL pro_userlog('Customer Receipts','{button1.text}')");
        }      
        catch (e:Exception)
    {   var msg:String="Class : ReportCustomerReceipts method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
