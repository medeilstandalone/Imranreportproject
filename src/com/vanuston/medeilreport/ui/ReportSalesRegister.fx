...
public class ReportSalesRegister
{
...
var log:Logger=Logger.getLogger(ReportSalesRegister.class,"Reports");
var dir1 = new File(".");
...
var sd:String;
var ed:String;
var parambill:String;
var billtype: String[]=["--select--","Cash Bill","Credit Bill","Card Bill","Counter Bill"];
    ...
    public-read def cbobilltype: javafx.scene.control.ChoiceBox = javafx.scene.control.ChoiceBox {
    ...
    public-read def stockhyperLink: javafx.scene.control.Hyperlink = javafx.scene.control.Hyperlink {
        layoutX: bind C4LX
        layoutY: bind bottonLY-40
        effect: null
        graphic: imageView3
        text: "Sales Items Stock"
        font: Arial_Bold_16
        graphicHPos: javafx.geometry.HPos.RIGHT
        action: stockhyperLinkAction
    }
    ...
    public-read def PanelSalesRegister: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, label5, label10, txtFromDate, txtToDate, button1, imgFrom, imgTo, FromPanel, label2, cbobilltype, hyperlink, stockhyperLink, panelAlert, panel, ]
    }
    ...
    function stockhyperLinkAction(): Void {
        var billparam:String;
        billparam=cbobillsel();
        ...
        else if(billparam.equals("--select--")) {
        FXinfo("Please select the bill type");
        }
        else
        {
        try
        ...
        params.put("type1",'salesstock');
        params.put("billtype",billparam);       
        params.put("REPORTS_DIR",image);        
        RepViewer.showReport("/reportfiles/sales_stock_statement.jasper",params);
        commonController.queryExecution("CALL pro_userlog('Sales Register','{stockhyperLink.text}')");
        }
        catch(e:Exception) {
           var msg:String="Class : ReportSalesRegister method : stockhyperLinkAction()   = {e.getMessage()}";
            log.debug(msg);
        }
        }
         }
    function cbobillsel():String
    {
        if(cbobilltype.selectedItem.toString()==billtype[1])
        {
           parambill="Cash_Bill";
        }
        else if(cbobilltype.selectedItem.toString()==billtype[2])
        {
           parambill="Credit_Bill";
        }
        else if(cbobilltype.selectedItem.toString()==billtype[3])
        {
           parambill="CD_Card_Bill";
        }
        else if(cbobilltype.selectedItem.toString()==billtype[4])
        {
           parambill="Account";
        }
        else if(cbobilltype.selectedItem.toString()==billtype[0])
        {
           parambill="--select--";
        }
    ...
    function hyperlinkAction(): Void {
        ...
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/salesregister.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Sales Register','{hyperlink.text}')");
         }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Sales Register",msg);
        showAlertbox();
        }
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
       ...
       frmFlag=false;
        ...
        {
        params.put("type1",'salesbytype');
        params.put("billtype",billparam);
        }
        params.put("REPORTS_DIR",image);
        
        RepViewer.showReport("/reportfiles/salesregister.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Sales Register','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportSalesRegister method : button1Action()   = {e.getMessage()}";
        log.debug(msg);        
    }
     }
   }
...
function run (): Void {
    var design = ReportSalesRegister {};
    design.Startup();
    javafx.stage.Stage {
