...
var log:Logger=Logger.getLogger(ReportProfitLoss.class,"Reports");
var sd:String;
var ed:String;
var dir1 = new File(".");
...
var commonController = RegistryFactory.getClientStub("CommonImplementStub") as CommonImplements;
    ...
    public-read def PanelProfitLoss: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("FR8".trim())){
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Profit and Loss",msg);
        showAlertbox();
        }
    ...
    function imgFromOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
       ...
       toFlag=false;
        ...
        commonController.querySelection("call pro_pandl('{sd}','{ed}')");
        ...
        RepViewer.showReport("/reportfiles/pandl.jasper",params);
        commonController.queryExecution("CALL pro_userlog('Profit and Loss','{button1.text}')");
        }
        
    catch (e:Exception)
    {
        var msg:String="Class : ReportProfitLoss method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
