...
var fxCalendar = FXCalendar {
    ...
    function rectangle2OnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
        ...
        CustomAlert.ShowInfo("Cancel Bill Report",msg);
        showAlertbox();
        }
    ...
    function imgFromOnMouseClicked(event: javafx.scene.input.MouseEvent): Void {
        ...
        RepViewer.showReport("/reportfiles/salescancelbill.jasper",params1);        
        }              
        }
    catch (e:Exception)
    {   var msg:String="Class : ReportCancelBill method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
        
    }
    commonController.queryExecution("CALL pro_userlog('Cancelled Bill Report','{button1.text}')");
   }
