...
public class ReportPurchaseDetail
{
...
var log:Logger=Logger.getLogger(ReportPurchaseDetail.class,"Reports");
var dir1 = new File(".");
...
var ed:String;
    ...
    public-read def label: javafx.scene.control.Label = javafx.scene.control.Label {
        ...
        effect: null
        text: "Purchase by Product Details "
        font: Arial_25
    ...
    public-read def PanelPurchaseDetail: javafx.scene.layout.Panel = javafx.scene.layout.Panel {
        content: [ label, label5, label10, txtFromDate, txtToDate, button1, imgFrom, imgTo, FromPanel, hyperlink, hyperlink2, panelAlert, panel, ]
    }
    ...
    function hyperlink2Action(): Void {
        ...
        params.put("fromdate",sdf1.format(sdf.parse(txtFromDate.text)));
        params.put("todate",sdf1.format(sdf.parse(txtToDate.text)));
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/purchase_adjustment.jasper",params);
        commonController.queryExecution("CALL pro_userlog('Purchase Details','{button1.text}')");
        }
         }
         catch(e:Exception) {
          var msg:String="Class : ReportPurchaseDetail method : hyperlink2Action()  = {e.getMessage()}";
          log.debug(msg);
         }
         }
    ...
    var KyEvent=bind CommonDeclare.currKyEvent on replace{
            if(CommonDeclare.currModule==7 and CommonDeclare.currRep.equals("PurR1".trim())){
    ...
    function hyperlinkAction(): Void {
        ...
        params.put("type1",'todaypurchase');
        params.put("distname",'');
        params.put("REPORTS_DIR",image);
        RepViewer.showReport("/reportfiles/purchase_detailreport.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Purchase Detail Report','{hyperlink.text}')");
        }
    ...
    function txtFromDateOnKeyPressed(event: javafx.scene.input.KeyEvent): Void {
        ...
        CustomAlert.ShowInfo("Purchase Detail",msg);
        showAlertbox();
        }
    ...
    function imgToOnMouseClicked (event: javafx.scene.input.MouseEvent): Void {
        ...
        params.put("type1",'purchasebydate');
        params.put("REPORTS_DIR",image);
        params.put("distname",'');
        RepViewer.showReport("/reportfiles/purchase_detailreport.jasper",params);
       commonController.queryExecution("CALL pro_userlog('Purchase Detail Report','{button1.text}')");
        }
     catch (e:Exception)
    {
        var msg:String="Class : ReportPurchaseDetail method : button1Action()   = {e.getMessage()}";
        log.debug(msg);
