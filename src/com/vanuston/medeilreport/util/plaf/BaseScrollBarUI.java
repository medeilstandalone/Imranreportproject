...
public class BaseScrollBarUI extends BasicScrollBarUI {
    ...
    protected void paintTrack(Graphics g, JComponent c, Rectangle trackBounds) {
        ...
        int h = c.getHeight();
        if (scrollbar.getOrientation() == JScrollBar.VERTICAL) {
            NewClass.fillVerGradient(g, AbstractLookAndFeel.getTheme().getTrackColors(), 0, 0, w, h);
        } else {
            NewClass.fillHorGradient(g, AbstractLookAndFeel.getTheme().getTrackColors(), 0, 0, w, h);
        }
    }
    ...
    protected Color[] getThumbColors() {
        ...
        } else if (!NewClass.isActive(scrollbar)) {
    ...
    protected void paintThumb(Graphics g, JComponent c, Rectangle thumbBounds) {
        ...
        if (scrollbar.getOrientation() == JScrollBar.VERTICAL) {
            NewClass.fillVerGradient(g, colors, 1, 1, thumbBounds.width - 1, thumbBounds.height - 1);
            NewClass.draw3DBorder(g, frameColorLo, ColorHelper.darker(frameColorLo, 15), 0, 0, thumbBounds.width, thumbBounds.height);
        ...
        {
            NewClass.fillHorGradient(g, colors, 1, 1, thumbBounds.width - 1, thumbBounds.height - 1);
            NewClass.draw3DBorder(g, frameColorLo, ColorHelper.darker(frameColorLo, 10), 0, 0, thumbBounds.width, thumbBounds.height);
            g.setColor(frameColorHi);
            g.drawLine(1, 1, thumbBounds.width - 2, 1);
            g.drawLine(1, 1, 1, thumbBounds.height - 2);
