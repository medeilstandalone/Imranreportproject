...
public class BaseTableHeaderUI extends BasicTableHeaderUI {
    ...
    protected void paintBackground(Graphics g, Rectangle cellRect, int col) {
        ...
        if (col == rolloverCol && component.isEnabled()) {
            NewClass.fillHorGradient(g, AbstractLookAndFeel.getTheme().getRolloverColors(), x, y, w, h);
        } else if (NewClass.isFrameActive(header)) {
            NewClass.fillHorGradient(g, AbstractLookAndFeel.getTheme().getColHeaderColors(), x, y, w, h);
        } else {
            NewClass.fillHorGradient(g, AbstractLookAndFeel.getTheme().getInActiveColors(), x, y, w, h);
        }
    }
